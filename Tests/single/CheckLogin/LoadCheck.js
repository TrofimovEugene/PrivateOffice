...
async function getCheckScreen() {
  ...
  try {
    ...
    await page.screenshot({
      path: 'Проверка входа учителя.png'
    });
    ...
    await page.screenshot({
      path: 'Проверка входа студента.png'
    });
    ...
    await page.screenshot({
      path: 'Заполненное окно регистрации.png'
    });
    await page.waitFor(1000);
    ...
    page.keyboard.type(user.password)
    await page.screenshot({
      path: 'Заполненное окно входа.png'
    });
    await page.waitFor(1000);
    await page.click('#nav-teacher > form > div.d-flex.justify-content-between > input.btn.btn-primary')
    ...
    await page.waitFor(1000);
    await page.screenshot({
      path: 'Личный кабинет учителя.png'
    });
