...
async function getCheckScreen() {
  ...
  await page.waitFor(1000);
  await page.screenshot({path: './result/Окно регистрации.png'});
  ...
  await page.screenshot({path: './result/Заполненное окно регистрации.png'});
  await page.waitFor(1000);
  ...
  page.keyboard.type(user.password)
  await page.screenshot({path: './result/Заполненное окно входа.png'});
  await page.waitFor(1000);
  await page.click('#nav-teacher > form > div.d-flex.justify-content-between > input.btn.btn-primary')

  await page.waitFor(1000);
  await page.screenshot({path: './result/Личный кабинет учителя.png'});
